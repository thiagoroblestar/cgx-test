...
export default async function handler(
    ...
    let authorId = parseInt(Array.isArray(authorIdParam) ? authorIdParam[0] : authorIdParam);

    let filteredQuotes = quotes;
    if (!isNaN(authorId)) {
        filteredQuotes = quotes.filter(quote => quote.authorId === authorId);
    }
    ...
    if (filteredQuotes.length > 0) {
        ...
        res.status(404).json({ success: false, message: "No quotes found for the provided author ID." });
    }
}
